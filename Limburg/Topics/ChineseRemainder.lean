...
This topic file is copied from Patrick Massot's "Glimpse of Lean" project
...
choose a representative `x' : R` for this quotient element. For
this the lemma `Ideal.Quotient.mk_surjective` is useful.
(Use `#check` to inspect this lemma. Why is it named like this?)
...
theorem kerLift_injective' (f : R →+* S) (x : R ⧸ ker f) (hx : kerLift f x = 0) : x = 0 := by
  obtain ⟨x', hx'⟩ := Ideal.Quotient.mk_surjective x
  rw [← hx']
  rw [← hx'] at hx
  sorry
...
`⨅ i, I i`. Be careful that, depending on the font you are using, the intersection symbol `⨅`
and the product `Π` could be tricky to distinguish.
-/
...
lemma chineseMap_mk (x : R) :
    chineseMap I (Ideal.Quotient.mk _ x) = fun i : ι ↦ Ideal.Quotient.mk (I i) x :=
  rfl

lemma chineseMap_mk' (x : R) (i : ι) :
    chineseMap I (Ideal.Quotient.mk _ x) i = Ideal.Quotient.mk (I i) x :=
  rfl
...
The union `s ∪ {i}` is spelled `insert i s`. The following lemmas about this operation will
be useful:
-/
...
lemma coprime_iInf_of_coprime {I : Ideal R} {J : ι → Ideal R} {s : Finset ι} :
  ...
  | @insert i s _ hs =>
      intro h
      rw [Finset.iInf_insert, inf_comm, Ideal.one_eq_top, eq_top_iff, ← Ideal.one_eq_top]
      set K := ⨅ j ∈ s, J j
      sorry
...
lemma chineseMap_surjective [Finite ι] {I : ι → Ideal R} (hI : ∀ i j, i ≠ j → I i + I j = 1) :
  ...
  have key : ∀ i, ∃ e : R,
      Ideal.Quotient.mk (I i) e = 1 ∧ ∀ j, j ≠ i → Ideal.Quotient.mk (I j) e = 0 := by
    intro i
